(function () {
  ...
  function renderCharts() {
    ...
    const meterAttributes = (study, label, value, maximum, valueText) => `role="meter" aria-label="${escapeHtml(`${studyIdentityLabel(study)}: ${label}`)}" aria-valuemin="0" aria-valuemax="${number(maximum, 2)}" aria-valuenow="${number(value, 2)}" aria-valuetext="${escapeHtml(valueText)}"`;
    ...
    $("#ratioChart").innerHTML = comparison.map((study) => {
      ...
      return `<div class="bar-row${current.className}" role="listitem" aria-label="${escapeHtml(`Study ${study.index}, ${studyShortName(study)}: length to span ratio; ${axisLabel}; ${statusLabel} (${status}) record: ${studyStatusDescription(study)}; Source: ${studySource(study)}; ${studySourceNote(study)}`)}"${current.description}><span class="bar-label" aria-hidden="true"><span class="chart-study-name"><span class="chart-study-index" aria-hidden="true">${escapeHtml(study.index)}</span><span class="chart-study-name-text">${escapeHtml(studyShortName(study))}</span></span><span class="chart-study-meta"><span class="chart-axis-context" title="${escapeHtml(axisLabel)}">${escapeHtml(axisLabel)}</span><span class="chart-status" data-status="${escapeHtml(status)}" title="${escapeHtml(studyStatusDescription(study))}">${escapeHtml(statusLabel)}</span>${current.cue}</span></span><span class="bar-track" ${meterAttributes(study, "Relative length to span ratio", ratio, maxRatio, `${number(ratio, 2)} of ${number(maxRatio, 2)} maximum`)}><i class="bar-fill" style="width:${(ratio / maxRatio) * 100}%"></i></span><span class="bar-value" aria-hidden="true">${number(ratio, 2)}</span></div>`;
    }).join("");
    $("#heightChart").innerHTML = comparison.map((study) => {
      ...
      const axisLabel = studyAxisLabel(study);
      const current = currentStudyContext(study);
      return `<div class="bar-row${current.className}" role="listitem" aria-label="${escapeHtml(`Study ${study.index}, ${studyShortName(study)}: height to span ratio; ${axisLabel}; ${statusLabel} (${status}) record: ${studyStatusDescription(study)}; Source: ${studySource(study)}; ${studySourceNote(study)}`)}"${current.description}><span class="bar-label" aria-hidden="true"><span class="chart-study-name"><span class="chart-study-index" aria-hidden="true">${escapeHtml(study.index)}</span><span class="chart-study-name-text">${escapeHtml(studyShortName(study))}</span></span><span class="chart-study-meta"><span class="chart-axis-context" title="${escapeHtml(axisLabel)}">${escapeHtml(axisLabel)}</span><span class="chart-status" data-status="${escapeHtml(status)}" title="${escapeHtml(studyStatusDescription(study))}">${escapeHtml(statusLabel)}</span>${current.cue}</span></span><span class="bar-track" ${meterAttributes(study, "Relative height to span ratio", ratio, maxHeightRatio, `${number(ratio, 2)} of ${number(maxHeightRatio, 2)} maximum`)}><i class="bar-fill teal" style="width:${(ratio / maxHeightRatio) * 100}%"></i></span><span class="bar-value" aria-hidden="true">${number(ratio, 2)}</span></div>`;
    }).join("");
    $("#moduleChart").innerHTML = comparison.map((study) => {
      ...
      const current = currentStudyContext(study);
      return `
      <div class="module-row${current.className}" role="listitem" aria-label="${escapeHtml(`Study ${study.index}, ${studyShortName(study)}: ${study.bayCount} bays, module length; ${axisLabel}; ${statusLabel} (${status}) record: ${studyStatusDescription(study)}; Source: ${studySource(study)}; ${studySourceNote(study)}`)}"${current.description}>
        <span class="module-name" aria-hidden="true"><span class="chart-study-name"><span class="chart-study-index" aria-hidden="true">${escapeHtml(study.index)}</span><span class="chart-study-name-text">${escapeHtml(studyShortName(study))}</span></span><span class="chart-study-meta"><span class="chart-axis-context" title="${escapeHtml(axisLabel)}">${escapeHtml(axisLabel)}</span><span class="chart-status" data-status="${escapeHtml(status)}" title="${escapeHtml(studyStatusDescription(study))}">${escapeHtml(statusLabel)}</span>${current.cue}</span></span>
        <span class="module-bars" ${meterAttributes(study, "Relative module length", study.module, maxModule, `${number(study.module)} ${unit} of ${number(maxModule)} ${unit} maximum`)} style="width:${moduleWidth}%">${Array.from({ length: study.bayCount }, () => '<i class="module-bar"></i>').join("")}</span>
