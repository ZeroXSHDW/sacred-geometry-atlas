(function () {
  ...
  function svgBase(study, title) {
    ...
    const floorArea = floorAreaReading(study);
    const volume = volumeReading(study);
    const description = `${surface} ${state.mode} ${studyStatusLabel(study).toLowerCase()} drawing showing the ${layer} for ${study.name} at ${zoomPercent()} zoom. Overall dimensions are length ${study.length} ${unitName}, span ${study.span} ${unitName}, and height ${study.height} ${unitName}. Envelope: ${study.envelope}. Axis: ${studyAxisLabel(study)}. Rhythm: ${study.bayCount} bays at a ${number(study.module)} ${unitName} module. Primary radius: ${number(study.radius)} ${unitName}. Symmetry index: ${number(study.symmetry, 2)}. Reading: ${reading || "No interpretive reading supplied."} Data status: ${studyStatusLabel(study).toLowerCase()} (${studyStatus(study)}); ${studyStatusDescription(study)} Source: ${studySource(study)}; ${studySourceNote(study)} Reference: ${study.churchName || study.name}. Floor area: ${floorArea.value}; basis: ${floorArea.basis}. Estimated volume: ${volume.value}; basis: ${volume.basis}.`;
