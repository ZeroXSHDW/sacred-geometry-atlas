...
- The no-script collection index keeps each study name as a direct jump to its static fragment record, explicitly associates that link with the visible study metadata, and explains that interactive drawings require JavaScript; the same hash remains a useful route handoff when JavaScript is restored or enabled.
