...
- Focused comparisons keep their selected-study strip visible, with inline removal, per-study axis, status, and source/provenance context, a current-Atlas-study cue, and clear-all controls that preserve keyboard focus.
...
- Active study and comparison views also keep their direct route visible on screen, so the reproducible hash link is discoverable before a share, citation, or print action.
